...
def create_racer_abbreviations_dict(file_name):
    ...
    with open(file_name, 'r') as fn:
        abbreviations_dict = ast.literal_eval(fn.read())
...
def sorted_individual_results(start_timings_, end_timings_, reverse_order=False):
    """
    ...
    lap_results = {key: end_timings_.get(key, datetime.datetime.now()) - start_timings_[key]
...
def timedelta_to_str(timedelta_value):
    ...
    return f"{str(minutes).zfill(2)}:{str(seconds).zfill(2)}:{str(timedelta_value.microseconds).zfill(6)}"


# prints result board to a console
def print_result_board(sorted_lap_results_, abbr_dict_):
    counter = 1
    for key, value in sorted_lap_results_.items():
        driver_data = abbr_dict_.get(key, ("Unknown driver", "Unknown team"))
        racer_name = driver_data[0]
        team_name = driver_data[1]
        best_time = timedelta_to_str(value)
...
print_result_board(sorted_lap_results, abbr_dict)
